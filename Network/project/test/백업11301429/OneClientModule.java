...
class OneClientModule extends Thread{
	...
	*/	
		...
		try{
			...
			int index = server.clientCList.size();	//0
			server.cc = new ClientCharacter(nickName, index, server.score, color[index]);
			...
			while(true){
				...
				if(proto.equals("#007")){	//턴관리
					index = server.clientCList.size();
					server.pln("@009"+(index-1)+" "+server.turn);
					if(server.turn == (index-1)){ 
						server.turn = 0;
					}else{
						server.turn += 1;
					}
					broadcast("@009"+server.turn);
					
				}
				if(proto.equals("#008")){	//게임종료했을때.
			...
			String msgE = nickName+"님 접속종료 "+ server.clientMap.size();
			broadcast(msgE);
			server.pln(msgE);
			/*예외처리 어떻게?*/
		}finally{	
